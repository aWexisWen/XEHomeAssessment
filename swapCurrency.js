...
async function convertBtn() {
    ...
    console.log("Clicked convert button");
      ...
      // Wait a bit for the correct selection to appear
      await driver.sleep(3000);  // Adjust the delay if necessary

    // Click swap button
    let swapButtonClass = 'sc-64b15396-2 uhdeD';
    await driver.wait(until.elementLocated(By.className(swapButtonClass)), 10000);
    let swapButton = await driver.findElement(By.className(swapButtonClass));
    await swapButton.click();
    console.log("Clicked swap button");

    // Optional: Verify that the swap occurred correctly
    let swappedFromCurrency = await driver.findElement(By.id("midmarketFromCurrency")).getAttribute("value");
    let swappedToCurrency = await driver.findElement(By.id("midmarketToCurrency")).getAttribute("value");
    console.log("Swapped currencies: From =", swappedFromCurrency, "To =", swappedToCurrency);
